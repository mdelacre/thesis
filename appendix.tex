...
\color{blue}Welchâ€™s statistic (\(W\)) \color{black}

\textbf{p.22} : In the numerator of the \sout{W-test} \color{blue}\(W\)
...
\item
  p.27 : ``Moreover, there is one constant observation in our
  ...
  \sout{negative} \textbf{positive} correlation between the \emph{SD}
  and the \sout{mean} \textbf{sample size} (meaning that the
  \textbf{largest} group \sout{with the highest mea} has the \sout{lowe}
  \textbf{largest} variance).''
\end{itemize}
...
\tightlist
\item
  p.39: titre de la Table 1: ``\sout{Expectency} \textbf{Expectation},
  bias and variance of Cohen's \sout{\(d_s\)} \(d\)''
\item
  p.39: dans cette mÃªme table, les incides \(_s\) doivent Ãªtre
  supprimÃ©s dans la premiÃ¨re colonne.
\item
  p.41: les mÃªmes remarques (concernant le titre et les indices)
  s'appliquent Ã~ la table 2.
\item
  p.46: les mÃªmes remarques (concernant les indices) s'appliquent Ã~ la
  table 3. De plus, il convient d'Ã©crire ``\sout{Cohen's} Hedges'
  \(g^*\)''.
\end{itemize}
\end{appendix}
