...
Deuxièmement, dans le contexte d’études de réplication, il est possible de se baser sur les résultats d’une étude antérieure pour définir les bornes d’équivalence. Deux solutions ont été proposées, respectivement par @simonsohn_p-curve_2014 et par @lakens_equivalence_2018, toutes deux visant à remettre en question la pertinence des tailles d'échantillons récoltées dans une étude d'origine. La solution proposée par @simonsohn_p-curve_2014 consiste à déterminer quelle taille d’effet l’étude d’origine aurait pu détecter avec une puissance de $33\%$, et à utiliser cette information pour définir les bornes d’équivalence. Par exemple, si l’étude d’origine comparait deux échantillons indépendants, sur base d’un test $t$ de Student, et que chaque échantillon était composé de 21
sujets, alors cette étude aurait eu une puissance de 33% pour détecter un effet $d$ de Cohen égal à .48. Si lors de la réplication, on obtient un effet significativement inférieur à .48, alors on en déduira que l’outil d’origine n’était pas apte à détecter proprement cet effet. Dans ce contexte, démontrer l’équivalence ne permet plus de rejeter la présence d’un effet pertinent, mais bien de rejeter la pertinence d’un outil utilisé en vue de mettre cet effet en évidence(Lakens, Scheel, & Isager, 2018). Dans le même ordre d'idée, la solution proposée par @lakens_equivalence_2018 est de définir, comme borne d'équivalence, le plus petit effet que l'étude d'origine aurait pu détecter comme étant significative. L'idée sous-jacente est la suivante: même lorsqu'un auteur ne statue pas explicitement sur ce qu'il considère comme étant un effet pertinent, il le fait au moins implicitement à travers la taille des échantillons récoltés. En effet, un effet ne pourra être détecté comme étant significatif que si la statistique de test (et donc, la taille d'effet basée sur la même quantité) dépasse une certaine valeur seuil (appelée la valeur critique), or, cette dernière dépend de la taille des échantillons récoltés. Par exemple, lorsqu'on compare les moyennes de deux groupes de sujets (constitués de 30 sujets par chacun) à l'aide d'un test $t$ de Student, l'effet ne sera jugé significatif que si la statistique de test dépasse 2.045, ce qui correspond à une statistique $d$ de Cohen de .373 (le lien entre les statistiques $t$ de Student et $d$ de Cohen ont été détaillées dans la chapitre 4). Dès lors, concrètement, si lors de la réplication, on obtient un effet inférieure à .373, on en déduira que la taille d'échantillon de l'étude d'origine était insuffisante pour étudier l'effet suggéré. 

Troisièmement, il est possible de se baser sur les ressources dont on dispose en vue de définir les bornes d'équivalence. *Si moi je ne suis pas capable d'avoir un échantillon de plus de 2000 personnes, il y a certains effets que je ne serai pas capable de calculer. Et donc, je peux utiliser cette taille d'échantillon pour déterminer la taille d'effet dont je suis certain que je pourrai raisonnablement conclure au rejet de l'hypothèse nulle. Et donc là, si on démontre qu'il y a équivalence, on ne tire pas la ccl que l'effet n'est pas pertinent, mais que cet effet qu'on a envie d'étudier ne peut être l'être sur base des tailles d'échantillon qu'on a l'habitude d'utiliser.*
